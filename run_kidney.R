...
# Set directory path for kidney data
dir <- "human_adult_kidney"

# Read miRNA-gene interaction data for kidney
...
wilcox_test_vg <- function(vg) {
  ...
  violin_box <- ggplot(
    ...
    labs(
      ...
      title = paste0("Kidney Gene Expression Noise Comparison (", vg, ")"),
...
p5 <- ggplot(gene_stats,
  ...
  labs(title = "Distribution of miRNA Target Numbers in Kidney Genes",
...
p6 <- ggplot(
  ...
  labs(
    ...
    title = "Expression Noise Comparison by miRNA Target Number (Kidney)",
    subtitle = paste("Wilcoxon test:", w_label, ",", p_label)
  ) +
  theme_classic() +
  theme(
...
mirna_high_expr <- mirna_gene_kidney %>%
...
mirna_gene_filtered_kidney <- mirna_gene_kidney %>%
  filter(miRNA %in% mirna_high_expr) %>%
  arrange(desc(exp.mean))

# Get top 5 most highly expressed miRNAs and save to file
mirna_gene_liver_5 <- mirna_gene_filtered_kidney[1:5,]
